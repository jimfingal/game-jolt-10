...
class DialogueInstanceEditor extends Editor {
	...
	function OnInspectorGUI() {
		...
		switch (target.phase) {
			case 0:
				...
				break;
			case 1:
				if(!target.dialogueSkin)
					GUILayout.Box("Be sure to set the skin in the inspector before previewing!");
				ShowHistory();
	...
	function ShowEntryEditor (i:int) {
		...
		if(!target.defaultAudio) {
			...
			if(GUILayout.Button("Add a Clip"))
				AddClip(i);
		}
		...
		switch (dialogue[i].mode) {
			...
			case 2:
				...
				if(dialogue[i].passwords) {
					...
					if(GUILayout.Button("->", GUILayout.Width(24)))
						GoTo(dialogue[i].exit.next);
					GUILayout.EndHorizontal();
				}
				if(GUILayout.Button("Add Password"))
					AddPassword(i);
				GUILayout.EndVertical();
				GUILayout.EndHorizontal();
				break;
	...
	function DeleteLink (i:int,j:int) {
		var dialogue = target.dialogue;
		var arr :
		Array = new Array(dialogue[i].links);
		arr.RemoveAt(j);
		dialogue[i].links = arr.ToBuiltin(Link);
	}
	...
	function DeleteChoice (i:int,j:int) {
		var dialogue = target.dialogue;
		var arr :
		Array = new Array(dialogue[i].choices);
		arr.RemoveAt(j);
		dialogue[i].choices = arr.ToBuiltin(DialogueChoice);
	}
	...
	function DeletePassword (i:int,j:int) {
		var dialogue = target.dialogue;
		var arr :
		Array = new Array(dialogue[i].passwords);
		arr.RemoveAt(j);
		dialogue[i].passwords = arr.ToBuiltin(DialogueChoice);
	}
	...
	function GoBack() {
		...
		var arr :
		Array = new Array(target.history);
		arr.Pop();
	...
	function ShowHistory() {
		...
		if(target.history) {
			if(target.history.length > 0) {
				...
				if(t.length > 40)
					t = t.Substring(0, 40) + "...";
				if(GUILayout.Button("<- [" + i + "] " + t, GUILayout.Width(Screen.width - 140)))
					GoBack();
			}
		}
		if(target.curItem != 0)
	...
	function AddClip (i:int) {
		var dialogue = target.dialogue;
		var arr :
		Array = new Array(dialogue[i].narration);
		if(target.defaultAudio) {
